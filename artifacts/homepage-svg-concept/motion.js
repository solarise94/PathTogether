(function () {
  ...
  const scenes=[
    ...
    {camera:detail(a),travel:0,hold:6200,where:'标注 A',title:'留下第一处观察',body:'标注固定在组织坐标上。缩小或移动视野后，仍能回到同一个位置。',note:'a'},
    ...
    {camera:detail(b),travel:0,hold:6200,where:'标注 B',title:'记录第二处位置',body:'为区域 B 添加观察标注，形成可以回看的观察路径。',note:'b'},
    {camera:all,travel:2200,hold:4200,where:'回看',title:'把观察放回整体',body:'回到全景，两处标注仍留在原来的组织位置。下一轮从整体重新开始。'}
  ];
  const extra=document.createElement('li');extra.textContent='回看';$('steps').append(extra);
  const observations={
    a:'细胞围绕腔隙呈环状排列，细胞核的大小与方向略有差异。已记录区域 A 的观察位置。',
    b:'这处腺体轮廓与邻近区域有所不同，周围可见间质。已记录区域 B，便于回看比较。'
  };
  ...
  function paint() {
    ...
    if(scene.note) {
      const full=observations[scene.note];
      const title=scene.note==='a'?'区域 A · 环绕腔隙的细胞排列':'区域 B · 腺体与周围间质';
  ...
  function tick(now) {
    ...
    last=now;
    if(elapsed>=scenes[index].travel+scenes[index].hold){
