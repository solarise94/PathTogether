...
- 日期：2026-08-24
- 状态：**代码已合并并部署（2026-08-24，待验收）**。批次 A/B/C 与批次 D 的代码侧核查完成（§13），外部 bundle 重建、安装与线上核对已完成（§14）；仅剩浏览器 UI 九项手工验收（§10.4）未完成，完成前不得标「已实施」
...
- 2026-08-24 二轮 review 修复：回跳命中索引视角时先恢复 `currentSnapshotView` 为来源快照再导航（对齐正式插件），画面/青色当前视角框/倍率/可见观察集合四者一致；取消选中不回退；降级路径（无索引）仍只导航不恢复视角。测试补状态与青色框断言。
