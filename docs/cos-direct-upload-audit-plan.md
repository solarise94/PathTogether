...
> 执行版更新：2026-09-25（Asia/Shanghai）
> 状态：**实现前执行稿，Phase 0 进行中**。产品 COS 功能仍未实现或部署，capability 为 `off`；专用私有测试桶已用于授权和网络 PoC。本文把 10 GB 暂存池、等待与清理规则收敛为可直接拆任务的合同。
...
- 执行日期：2026-09-24 起（Phase 0 进行中，未完成）
- 执行人/Agent：ZCode
- 基线 commit：`a2d2250`（工具链 `40fdbb2`，子账号策略模板 `6f93bc1`）
- 测试 COS bucket/region：`histopilot-1255456712` / `ap-shanghai`（专用私有桶）
- 三段吞吐证据：300/500 MB 各段单次样本见 [Phase 0 证据 §9](evidence/cos-20260924.md)；2026-09-25 用户裁决单次样本足够（上传稳、下载不稳），不重复测量，分流政策维持默认平台 + 手动 COS。
- 授权决议：`A-presign-parts`（候选 B 因 §3.0 条件 3 失败，不实现 STS 上传 API）
- 证据：[`docs/evidence/cos-20260924.md`](evidence/cos-20260924.md) §7
- 授权证据：B 同一 STS 在有效期内把同一 key 写成两个版本，合计 4000 字节，大于假定预约 64 字节；A 在绑定 Content-Length 时超长/截短/改 partNumber/过期重放均被拒绝。不绑定长度时超长分块被接受，因此生产实现必须绑定 Content-Length。三个生产 origin 的 CORS 已通过；生产页面 CSP 正向验收未做（env 门控实现与本地机制验证见 evidence §10.1/§10.2），费用证据未做（evidence §10.3/§10.4）。
- **2026-09-25 顺序豁免（用户明确授权）**：Phase 0 生产 CSP/费用两项未关闭即开始 Phase 1–4 产品实现（"把设计文档实现掉"）；capability 全程 `off`，无生产凭证、无部署，实现索引 evidence §10.5。§1 四项 capability 门禁未过前不得置 available，此约束不变。
- 10 GB 容量对账证据：（待 Phase 5 演练）
- capability 状态：`off`（`internal` / `on` 尚未启用）
...
### 1.1 成熟组件优先与自研边界（2026-09-25）

**能用成熟组件就不自行重写。** 生产实现按“现有项目能力 → 官方 SDK → 兼容且维护活跃的开源组件 → 必要的薄适配”选择；不因 PoC 已写好脚本就直接沿用为产品基础设施。

- **COS 协议层：** 优先使用腾讯云官方服务端 SDK 完成签名、multipart、分页、版本操作和对象读写。`cos_client.py` 只封装项目配置、错误映射与预算约束，不自行维护签名算法、XML 协议或通用 COS 客户端。官方 SDK 能否正确签入 `Content-Length` 必须通过实际验收，不能仅凭“官方”认定满足合同。
- **浏览器通用能力：** 优先评估成熟上传组件（如 Uppy）的队列、进度、重试和取消能力，并复用现有页面。只有符合 A 合同的组件/配置才可接入；浏览器不得获得 STS 密钥或 Initiate/Complete 权限。具体候选与选择记录见[分流调研 §3](upload-routing-open-source-review.md#3-开源方案比较)。
- **项目业务逻辑：** 自有代码集中在容量预约、权限、任务状态机、调度续租、本地提交、Viewer readiness、清理策略及必要适配。SDK 的自动重试、并发、完成和取消行为仍须受任务状态、流量预算、截止时间与 fencing 约束。
- **例外要有依据：** 某项无法复用时，先记录评估组件及版本、官方文档依据、实测不兼容点与最小自有代码范围，再做薄适配；不得直接扩展成通用上传框架。不能通过退回 B、取消长度绑定或放宽 10 GB 规则来迁就组件。
- **依赖验收：** 记录选定版本、来源、许可证、维护状态及已知安全问题的检查结果；锁版本，浏览器依赖本地托管并校验哈希，不用 latest CDN。替换 PoC 实现后重新运行相关真实验收，旧 PoC 结果不能替代生产 SDK/组件证据。
...
文首已裁决为 A，生产实现本节与 §4.A；§3.0 保留历史选型判据，不重新启用 B。
...
Phase 0 已有真实账号和 COS 网络证据，详见文首链接；生产页面 CSP 正向 PUT、费用与桶生命周期配置仍待验收。不以 mock 替代发布证据。真实失败要记录退出条件。HAR 脱敏。
...
1. 按 §1.1 选用官方服务端 SDK，记录版本与适配边界；`cos_client.py` 不复制 PoC 的手写签名/XML 客户端。worker 只消费 DB 中服务端固定的 bucket/key/version；客户端回报只作提示。
...
1. 实现 §4 共用 API 与已裁决的 A `parts/sign + resume`；B `credentials` 接口必须不存在。预签名优先通过官方 SDK 生成，绑定长度及所有 §3.1 签名约束；重跑长度恰好/超长/截短、参数篡改、过期重放和越权方法的真实负向测试。
...
1. 按 §1.1 与分流调研 §3 先评估成熟上传组件，记录选用或不适用依据；能满足 A 合同则复用其通用上传能力，只写平台 API 薄适配。若组件无法满足约束或引入不必要复杂度，记录具体原因后用原生 XHR/fetch 实现最小 UploadPart 适配，不自行建设通用上传框架。不得接入 B 的 STS 高级上传路径。
...
- 交付组件选型记录、锁定版本与许可证、必要自研范围及理由；确认生产代码不导入 PoC 手写签名/协议实现，生产 SDK/组件的真实验收证据齐全。
