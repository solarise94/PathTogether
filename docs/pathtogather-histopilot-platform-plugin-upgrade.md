...
3. 让 AI 读片以插件形式按组织安装、授权、停用和升级。
4. 保证人工产生的业务数据不依赖任一 AI 插件而存在。
5. 对人工与 AI 操作提供可追溯的审计链。
6. 为未来其他算法插件预留受控能力，例如细胞计数、IHC 定量和质控。
...
- 不在第一版引入复杂的实时多人光标，可先完成评论、标注变更和在线状态。
...
- 组织与工作区；
- 成员邀请和停用；
- `owner / admin / pathologist / reviewer / viewer` 角色；
- 病例级额外授权；
- 服务账户和插件安装身份。

要求：

- 所有病例、切片、标注、分享和审计记录携带 `organization_id`；
- 默认拒绝跨组织访问；
- 后端做资源级鉴权，不能只依赖前端隐藏按钮。

#### 5.1.1 最小权限矩阵

以下矩阵是首版的后端授权基线。`本人`表示仅可操作自己创建且尚未进入锁定状态的对象；病例级显式授权可以收紧权限，但不能突破组织角色上限。

| 动作 | owner | admin | pathologist | reviewer | viewer |
|---|---:|---:|---:|---:|---:|
| 查看已授权病例/切片/标注 | ✓ | ✓ | ✓ | ✓ | ✓ |
| 创建病例 | ✓ | ✓ | ✓ | — | — |
| 编辑病例基本信息 | ✓ | ✓ | 已分配病例 | — | — |
...
| 删除标注 | ✓ | ✓ | 本人 | 本人 | — |
| 评论标注 | ✓ | ✓ | ✓ | ✓ | — |
| 提交 `draft → in_review` | ✓ | ✓ | 已分配病例 | — | — |
| 完成 `in_review → finalized` | ✓ | ✓ | — | 已分配病例 | — |
| 重新打开 finalized 病例 | ✓ | ✓ | — | — | — |
| 归档/恢复病例 | ✓ | ✓ | — | — | — |
| 管理组织成员 | ✓ | ✓ | — | — | — |
| 安装/升级/授权插件 | ✓ | ✓ | — | — | — |
| 运行已授权 HistoPilot | ✓ | ✓ | 已分配病例 | 已分配病例 | — |

病例状态对写操作具有更高优先级：

| Case 状态 | 标注/评论规则 | 插件规则 |
|---|---|---|
| `draft` | 按角色矩阵正常创建、编辑、删除、评论 | 允许运行并写入 AI 建议 |
| `in_review` | 允许 pathologist/reviewer 补充；所有变更进入审计 | 允许运行；AI 标注默认为待审核 |
| `finalized` | 默认只读；只有 owner/admin 先执行“重新打开”才能修改 | 禁止产生正式写回，可运行只读问答 |
| `archived` | 完全只读；恢复后才能修改 | 禁止启动新 run，只允许读取保留历史 |

annotation 的 `resolved` 仍是可审计对象，不等于删除。对 finalized/archived 病例的后台批量写入也必须遵守同一状态规则。
