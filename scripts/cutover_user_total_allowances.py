...
window → cutover 扫描不到未提交用户 → 建号提交授权面/override」）。
apply/rollback 在维护闸成功后全程持**会话级** ``pg_advisory_lock``（键
...
rollback-plan（§8.2；不是切 flag）：
  ...
  2. 持会话级用户开通锁（同 apply，与建号/兑换互斥；finally 显式释放）；
  3. 前置检查：reconcile_total_allowances 无 drift、open total holds=0、
     open 的 user 窗口目标 hold=0（否则回滚后 reserved=0 与 reconcile 的
     expected_reserved 不一致）、每个 priced event 恰一条 usage ledger
     （双扣检测）、hold 单目标；
  4. 同一 SERIALIZABLE 事务内：锁 users(role='user')，为每个 user 生成
     **新的**月窗口（边界由服务端 ``window_bounds("calendar_month",
     ...
     DELETE/清零）→ 提交；
  5. 维护闸**保持开启**：回滚事务提交后需人工验收（reconcile 窗口无
...
def preflight(conn, *, actor, at_dt=None):
    ...
    完整镜像 apply 的静态前置条件（review R2-F3）：target 设置行缺失/值
    非 "window"、维护闸已开、默认总额度不可解析、物化名单存在无策略
    user——均进 problems（ok=false 硬失败，不留假绿）。
    """
    at_dt = at_dt or datetime.now(timezone.utc)
    problems = []
    with _tx(conn) as c:
        with c.cursor() as cur:
            # 原生行读取（镜像 apply 的 CAS WHERE value='window'）：行缺失
            # → None。get_user_spend_target_tx 的「缺/非法回退 window」会
            # 掩盖「apply CAS 必然失败」的形态，preflight 不用它
            cur.execute(
                "SELECT value FROM platform_settings WHERE key=%s",
                (settings_store.USER_SPEND_TARGET_KEY,))
            target_row = cur.fetchone()
            target = target_row["value"] if target_row is not None else None
            if target is None:
                problems.append({"problem": "target_setting_missing"})
            elif target != "window":
                problems.append({"problem": "target_not_window",
            ...
            if maintenance is True:
                ...
                # 命中——与 target_setting_missing 同口径提前暴露
        ...
        try:
            ...
            with _tx(conn) as c:
                with c.cursor() as cur:
                    ...
                    # 5. CAS 切 target（同事务内单条 UPDATE 比较写入；未命中
...
def rollback_plan(conn, *, actor, at_dt=None):
    """回滚规划（§8.2）：不切 flag——维护闸+开通锁 → reconcile/双扣检测 →
    建新窗并切回（维护闸保持开启）。"""
    at_dt = at_dt or datetime.now(timezone.utc)
    ...
    if current_maintenance is True:
        ...
        _die("ai_dispatch_maintenance 存量值非法（%r）——人工核查"
             % (current_maintenance,))
    # 2. 会话级用户开通锁（同 apply：与建号/兑换事务互斥，全程持有；
    #    finally 显式释放——_die 抛 SystemExit 亦走 finally）
    _acquire_user_provisioning_lock(conn)
    try:
        try:
            # 3. 前置检查（只读）
            ...
            # 4. SERIALIZABLE：建新窗 + 冻结 allowance（投影退出授权）+ CAS 切回
            with _tx(conn) as c:
                with c.cursor() as cur:
                    cur.execute(
                        "SET TRANSACTION ISOLATION LEVEL SERIALIZABLE")
